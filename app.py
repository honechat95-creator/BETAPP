...
st.caption("Cuotas reales + modelo con datos reales + live monitor + alertas Telegram.")
...
def fetch_event_odds(sport_key: str, event_id: str):
...
def merge_score_info(events, scores):
    ...
    for e in events:
        ...
        if s:
            row["live_status"] = s.get("completed")
            row["score_obj"] = s.get("scores", [])
            row["score_home"] = None
            row["score_away"] = None
            try:
                sc = s.get("scores", [])
                if len(sc) >= 2:
                    row["score_home"] = sc[0].get("score")
                    row["score_away"] = sc[1].get("score")
            except Exception:
                pass
        merged.append(row)
...
try:
    with st.spinner("Cargando partidos y cuotas..."):
        ...
        scores = fetch_scores(league["odds_key"])
        events = merge_score_info(events, scores)

    if live_mode:
        events = [e for e in events if e.get("score_obj")]

    if not events:
        st.warning("No hay partidos disponibles con ese filtro.")
        st.stop()

    labels = {}
    for e in events:
        score_txt = ""
        if e.get("score_home") is not None and e.get("score_away") is not None:
            score_txt = f" | {e['score_home']}-{e['score_away']}"
        label = f"{e['home_team']} vs {e['away_team']} | {e.get('commence_time', 'N/D')}{score_txt}"
        labels[label] = e

    selected_label = st.selectbox("Partido", list(labels.keys()))

    c1, c2 = st.columns([1, 1])
    refresh_clicked = c1.button("Actualizar cuotas", use_container_width=True)
    analyze_clicked = c2.button("Analizar partido", use_container_width=True)

    if refresh_clicked or analyze_clicked:
        event = labels[selected_label]

        if refresh_clicked and event.get("id"):
            try:
                refreshed = fetch_event_odds(league["odds_key"], event["id"])
        ...
        if event.get("score_home") is not None and event.get("score_away") is not None:
            st.write(f"**Marcador live/recent:** {event['score_home']} - {event['score_away']}")
        ...
        if len(value_df):
            st.success(f"Se detectaron {len(value_df)} picks con value.")
        else:
            st.warning("No hay value claro con el umbral actual.")
